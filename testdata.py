...
def run_all():
    raw_test, raw_pos, calibration = do_test()
    position = get_position_data(raw_pos)
    merged = merge_data(raw_test, position)
    bigboy = make_big_array(merged, calibration)
    v1 = get_Velocity_Jono_version(bigboy)
    v1 = downwash_correction(v1)
    basic_quiver(v1)
